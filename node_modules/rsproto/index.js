...
rsproto.prototype.onMultiplexConnection = function(stream) {
  ...
  if (idm = stream.id.match(/^:[^:]+:exec:(.*)$/)) {
    ...
    forwardStream.pipe(child.stdin);
    child.stdout.pipe(forwardStream);
    child.stderr.pipe(forwardStream);
...
rsproto.loadConfig = function(configFile) {
  ...
  } catch(e) { console.log("Unable to read config from "+configFile+": ",e); return {}; }
}
