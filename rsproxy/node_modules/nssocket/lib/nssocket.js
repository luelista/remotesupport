...
var NsSocket = exports.NsSocket = function (socket, options) {
  ...
  //
  this.socket     = socket;
  this.connected  = options.connected || (socket && socket.writable && socket.readable) || false;
  ...
  this.retry      = {
    retries:  0,
    max:      options.maxRetries || 100,
    interval: options.retryInterval || 5000,
    wait:     options.retryInterval || 5000,
    calculateInterval: options.retryCalculateInterval || function(interval, retries) {
      return Math.pow(10, retries) * interval;
    }
  };
...
NsSocket.prototype.connect = function connect() {
  ...
  this._options.type = this._options.type || 'tcp4';
  
  this.connected = true;
  this.socket =  this._options.type === 'tls' 
    ? tls.connect(this._options, this._onStart.bind(this))
    : net.connect(this._options, this._onStart.bind(this));
...
NsSocket.prototype.reconnect = function reconnect() {
  ...
  if (this.reconnect.waiting) {  return; }
  ...
  function doReconnect() {
    ...
    self.socket.on('error', function(){}); //ignore errors of old socket
  ...
  this.retry.wait = this.retry.calculateInterval(this.retry.interval, this.retry.retries);
